...
const SkuContent = ({ item, isFirstItem, product, children }: Props) => {
  ...
  // Handler para cliques em elementos relacionados ao Numeric Stepper
  const handleClick = (e: SyntheticEvent) => {
    const clickedElement = e.target as HTMLElement;
    const isNumericStepperClicked = clickedElement.classList.value.includes('vtex-numeric-stepper');
  ...
  const containerClasses = classNames(
    ...
    {
      [`bw1 b--muted-1 ${handles.selectedSkuContentWrapper}`]: selectedItem.itemId !== item.itemId,
    },
    {
      [`bw1 b--blue ${handles.selectedSkuContentWrapper}`]: selectedItem.itemId === item.itemId,
    }
  );
